...
Log::~Log() {
    ...
    deque_->Close();    // 关闭队列
    writeThread_->join();   // 阻塞，等待当前线程完成手中的任务
...
void Log::AsyncWrite_() {
    ...
    while(deque_->pop(str)) {
        lock_guard<mutex> locker(mtx_);
        fputs(str.c_str(), fp_); // C语言库函数，把字符串写入到指定的流(stream)中，但不包括空字符，不自动写入字符串结束标记符‘\0’。
...
void Log::write(int level, const char *format, ...) {
    struct timeval now = {0, 0};    // 用于表示时间的结构体，通常在操作系统编程中使用。它包含两个字段，分别是秒数和微秒数。
    gettimeofday(&now, nullptr);    // 获取时间，精度可以达到微妙
    time_t tSec = now.tv_sec;       // 获取系统当前时间的秒数
    ...
    if (toDay_ != t.tm_mday || (lineCount_ && (lineCount_  %  MAX_LINES == 0)))
    {
        ...
        char tail[36] = {0};    // 日志每行起始部分36个字符：日期、等级
    ...
    // 在buffer内生成一条对应的日志信息
    {
        ...
        int m = vsnprintf(buff_.BeginWrite(), buff_.WritableBytes(), format, vaList); // 将格式化数据从可变参数列表写入缓冲区
        va_end(vaList);
